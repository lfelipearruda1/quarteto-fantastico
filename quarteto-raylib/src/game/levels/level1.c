...
static void InitLevel1(Level *level) {
    ...
    data->backgroundTexture = LoadTexture("assets/fundoicoisa.png");

    data->player.position = (Vector2){ 100, GROUND_Y - 60 };
    data->player.velocity = (Vector2){ 0, 0 };
    data->player.onGround = true;
    data->player.health = 3;
    data->player.width = 50;
    data->player.height = 60;
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        data->enemies[i].position = (Vector2){ 400 + i * 350, GROUND_Y - 40 };
    ...
    for (int i = 0; i < MAX_OBSTACLES; i++) {
        float x = 300 + i * 200 + (float)(rand() % 100);
        float w = 60 + (float)(rand() % 40);
        float h = 30 + (float)(rand() % 30);
        data->obstacles[i].rect = (Rectangle){ x, GROUND_Y - h, w, h };
...
static void UpdateLevel1(Level *level, GameState *state) {
    ...
    if (data->player.position.y >= GROUND_Y - data->player.height) {
        ...
        if (data->obstacles[i].active) {
            if (CheckCollisionRecs(playerRect, data->obstacles[i].rect)) {
                if (data->player.velocity.y > 0 &&
                    data->player.position.y + data->player.height - data->player.velocity.y <= data->obstacles[i].rect.y) {
                    data->player.position.y = data->obstacles[i].rect.y - data->player.height;
                    data->player.velocity.y = 0;
                    data->player.onGround = true;
                } else {
                    data->player.position.x = data->obstacles[i].rect.x - data->player.width - 2;
                    data->player.health--;
    ...
    for (int i = 0; i < MAX_ROCKS; i++) {
        if (data->rocks[i].active) {
            ...
            if (data->rocks[i].position.x > data->cameraX + W + 50) {
                data->rocks[i].active = false;
            }
        }
    }
...
static void DrawLevel1(Level *level) {
    ...
    DrawRectangle(0 - (int)data->cameraX, GROUND_Y, MAP_LENGTH + 1000, H - GROUND_Y, (Color){101, 67, 33, 255});

    for (int i = 0; i < MAP_LENGTH / 100; i++) {
        DrawRectangle(i * 100 - (int)data->cameraX, GROUND_Y - 5, 80, 5, (Color){76, 187, 23, 255});
    }
    ...
    for (int i = 0; i < MAX_OBSTACLES; i++) {
        if (data->obstacles[i].active) {
            ...
            DrawRectangleRec(drawRect, (Color){60, 60, 60, 255});
            DrawRectangleLinesEx(drawRect, 2, BLACK);
        }
    }

    Rectangle playerDrawRect = { 
        data->player.position.x - data->cameraX, 
        data->player.position.y, 
        data->player.width, 
        data->player.height 
    };
    DrawRectangleRec(playerDrawRect, ORANGE);
    DrawRectangleLinesEx(playerDrawRect, 3, (Color){180, 100, 20, 255});
    
    DrawRectangle(
        (int)(data->player.position.x - data->cameraX + 10), 
        (int)(data->player.position.y + 15), 
        10, 10, BLACK
    );
    DrawRectangle(
        (int)(data->player.position.x - data->cameraX + 30), 
        (int)(data->player.position.y + 15), 
        10, 10, BLACK
    );
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        if (data->enemies[i].active) {
            ...
            DrawRectangleRec(enemyDrawRect, RED);
            DrawRectangleLinesEx(enemyDrawRect, 2, DARKGRAY);
    ...
    DrawText("ESPACO: Pular | E/ENTER: Atirar Pedra", 10, H - 60, 16, LIGHTGRAY);
